...
import { calculateNSTEMITimi } from '../../data/mockPatients';
...
const NSTEMIForm = forwardRef(function NSTEMIForm(
  ...
  // TIMI Specifics
  const [nAge65, setNAge65] = useState(editingRecord?.timiDetails?.age65OrOver ?? patientAge >= 65);
  const [nChd, setNChd] = useState(editingRecord?.timiDetails?.atLeast3CHD_RiskFactors ?? false);
  const [nStenosis, setNStenosis] = useState(editingRecord?.timiDetails?.priorCoronaryStenosis50Plus ?? false);
  const [nSt, setNSt] = useState(editingRecord?.timiDetails?.stDeviationAtAdmission ?? false);
  const [nAngina, setNAngina] = useState(editingRecord?.timiDetails?.atLeast2AnginalEpisodesLast24h ?? false);
  const [nMarkers, setNMarkers] = useState(editingRecord?.timiDetails?.elevatedSerumCardiacMarkers ?? false);
  const [nAsa, setNAsa] = useState(editingRecord?.timiDetails?.aspirinUseLast7d ?? false);
  ...
  const timiScore = useMemo(() => {
    return calculateNSTEMITimi({
      age65OrOver: nAge65,
      atLeast3CHD_RiskFactors: nChd,
      priorCoronaryStenosis50Plus: nStenosis,
      stDeviationAtAdmission: nSt,
      atLeast2AnginalEpisodesLast24h: nAngina,
      elevatedSerumCardiacMarkers: nMarkers,
      aspirinUseLast7d: nAsa
    });
  }, [nAge65, nChd, nStenosis, nSt, nAngina, nMarkers, nAsa]);
  ...
  return (
    <div className="space-y-6">
      ...
      <SectionCard title={`TIMI Risk Stratification score (NSTEMI Criteria)`}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 text-xs text-slate-600">
          <label className="flex items-center gap-2 p-2 bg-white rounded border cursor-pointer">
            <input type="checkbox" checked={nAge65} onChange={(e) => setNAge65(e.target.checked)} />
            <span>Age &gt;= 65 years (+1 pt)</span>
          </label>
          <label className="flex items-center gap-2 p-2 bg-white rounded border cursor-pointer">
            <input type="checkbox" checked={nChd} onChange={(e) => setNChd(e.target.checked)} />
            <span>At least 3 CHD risk factors (+1)</span>
          </label>
          <label className="flex items-center gap-2 p-2 bg-white rounded border cursor-pointer">
            <input type="checkbox" checked={nStenosis} onChange={(e) => setNStenosis(e.target.checked)} />
            <span>Prior Coronary Stenosis &gt;50% (+1)</span>
          </label>
          <label className="flex items-center gap-2 p-2 bg-white rounded border cursor-pointer">
            <input type="checkbox" checked={nSt} onChange={(e) => setNSt(e.target.checked)} />
            <span>ST deviation at admission (+1)</span>
          </label>
          <label className="flex items-center gap-2 p-2 bg-white rounded border cursor-pointer">
            <input type="checkbox" checked={nAngina} onChange={(e) => setNAngina(e.target.checked)} />
            <span>At least 2 angina episodes last 24h (+1)</span>
          </label>
          <label className="flex items-center gap-2 p-2 bg-white rounded border cursor-pointer">
            <input type="checkbox" checked={nMarkers} onChange={(e) => setNMarkers(e.target.checked)} />
            <span>Elevated Cardiac Markers (+1)</span>
          </label>
          <label className="flex items-center gap-2 p-2 bg-white rounded border cursor-pointer">
            <input type="checkbox" checked={nAsa} onChange={(e) => setNAsa(e.target.checked)} />
