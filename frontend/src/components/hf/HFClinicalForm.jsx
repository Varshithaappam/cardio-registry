...
const HF_ETIOLOGY_CV = ['Ischemic', 'Dilated', 'Valvular', 'Toxic', 'Hypertension', 'Congenital', 'Myocarditis'];
...
const HFClinicalForm = forwardRef(function HFClinicalForm(
  ...
  const [drugRows, setDrugRows] = useState(
    editingRecord?.medicalTherapy?.drugs?.length ?
      editingRecord.medicalTherapy.drugs.map((drug, index) => ({
  ...
  const vBmi = !vUnableToWeigh && vWeight > 0 && vHeight > 0 ?
    Number((vWeight / Math.pow(vHeight / 100, 2)).toFixed(1)) :
    undefined;
  ...
  useImperativeHandle(ref, () => ({
    getSubmissionData,
    getCompletionPercent: () => completionPercent
  }));

  const updateDrugRow = (index, updatedDrug) => {
    setDrugRows((prev) => prev.map((row, rowIndex) => rowIndex === index ? updatedDrug : row));
  };

  const removeDrugRow = (index) => {
    setDrugRows((prev) => prev.length === 1 ? prev : prev.filter((_, rowIndex) => rowIndex !== index));
  };
    ...
    <div className="space-y-6">
      <SectionCard title="1. Patient Profile" subtitle="Master registry demographics and baseline comorbidities">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 text-xs">
          ...
          <div className="p-3 bg-white border border-slate-100 rounded-lg">
            <span className="text-slate-400 font-semibold uppercase block">Insurance Mode</span>
            <span className="text-slate-800 font-bold block mt-1">{patient.insuranceMode || 'N/A'}</span>
          </div>
          <div className="p-3 bg-white border border-slate-100 rounded-lg">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <FormField label="Treating Cardiologist" required>
            <select
              ...
              onChange={(e) => setTreatingCardiologist(e.target.value)}
              className="w-full p-2 border border-slate-200 rounded-lg text-sm bg-white">
              {CARDIOLOGISTS.map((doctor) => <option key={doctor} value={doctor}>{doctor}</option>)}
            </select>
          </FormField>
          <FormField label="Referring Doctor">
            <select
              ...
              className="w-full p-2 border border-slate-200 rounded-lg text-sm bg-white">
              {REFERRING_DOCTORS.map((doctor) => <option key={doctor} value={doctor}>{doctor}</option>)}
            </select>
          </FormField>
        </div>
        <RadioGroup
      ...
      <SectionCard title="3. Initial Clinical Assessment" subtitle="Presentation, vitals, symptoms and baseline classification">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          {vUnableToWeigh &&
          <FormField label="Unable to Weigh Reason">
      ...
      <SectionCard title="6. Medical Therapy (Dose & Frequency)" subtitle="Guideline-directed medical therapy with dosing details">
        <div className="space-y-3">
          {drugRows.map((drug, index) =>
          <DrugDoseRow
            key={drug.id}
            drug={drug}
            onChange={(updatedDrug) => updateDrugRow(index, updatedDrug)}
            onRemove={() => removeDrugRow(index)}
            canRemove={drugRows.length > 1} />
          )}
        </div>
        <button
          type="button"
          onClick={() => setDrugRows((prev) => [...prev, emptyDrugRow()])}
          className="px-3 py-2 bg-teal-50 hover:bg-teal-100 text-teal-700 rounded-lg text-xs font-bold border border-teal-100">
          + Add Medication Row
        </button>
      ...
      <SectionCard title="7. Device Therapy" subtitle="Current implanted devices and eligibility assessment">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <FormField label="Recommended Brand / Model">
            <input id="hf-eligible-brand" type="text" value={eligibleDeviceBrand} onChange={(e) => setEligibleDeviceBrand(e.target.value)} className="w-full p-2 border border-slate-200 rounded-lg text-sm bg-white" />
          </FormField>
