...
export default function STEMIHistoryList({ regPatientId, patientName, onEditEventClick, isReadOnly = false }) {
  ...
  const fetchHistory = async () => {
    ...
    try {
      setLoading(true);
      const res = await api.get(`/stemi/history/${regPatientId}`);
      if (res.data && res.data.success) {
        setHistory(res.data.data || []);
      } else {
        throw new Error(res.data?.message || 'Failed to retrieve STEMI history');
      }
    } catch (err) {
      console.error('Error fetching STEMI history list:', err);
  ...
  if (loading) {
    return (
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 flex flex-col items-center justify-center min-h-[200px]">
        ...
        <p className="text-xs text-slate-400">Loading STEMI assessment history...</p>
      </div>
    );
  }
      ...
      <div className="space-y-2 max-h-[300px] overflow-y-auto pr-1">
          ...
          return (
            ...
            >
              <div className="flex items-center gap-3">
                <div className={`w-8 h-8 rounded-lg ${isDraftRecord ? 'bg-amber-100 text-amber-800 border-amber-300' : 'bg-red-50 text-red-700 border-red-100'} flex items-center justify-center text-xs font-bold border`}>
                  #{encounterNum}
                </div>
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-bold text-slate-800">
                      {record.acs_no || record.ip_no || `ID: ${record.stemi_id}`}
                    </span>
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={() => navigate(`/patient/${regPatientId}/view/${record.stemi_id}?formType=STEMI`, { state: { from: `/patient/${regPatientId}` } })}
                ...
                {isDeletedRecord ? (
                  <>
                    ...
                    {canDelete && (
                      <button
                        onClick={() => handleUndeleteRecord(record.stemi_id, record.acs_no || record.ip_no)}
                        ...
                        title="Restore / Undelete STEMI Record"
                      >
                  ...
                  <>
                    {isDraftRecord && (
                      <button
                        disabled={isReadOnly}
                        onClick={isReadOnly ? undefined : () => navigate(`/patient/${regPatientId}/edit/${record.stemi_id}?formType=STEMI`)}
                    ...
                    {canDelete && (
                      <button
                        onClick={() => handleDeleteRecord(record.stemi_id, record.acs_no || record.ip_no)}
        ...
        {history.length === 0 && (
          <div className="text-center py-8 text-slate-400 text-xs italic">
            No historical STEMI encounters found for this patient.
          </div>
        )}
      </div>
    </div>
