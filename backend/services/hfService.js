...
/**
 * Generate HF Registry Number
 * Format: REG-HF-00001
 */
async function generateHFRegistryNo(conn) {
    const [rows] = await conn.execute("SELECT COUNT(*) as count FROM hf_registry");
    const nextNum = (rows[0]?.count || 0) + 1;
    return `REG-HF-${String(nextNum).padStart(5, '0')}`;
}
...
async function saveHfAssessment(data) {
    ...
    try {
        ...
        // 1. Insert hf_registry
        const hf_registry_no = await generateHFRegistryNo(conn);
